(() => {
      ...
      if (preserveWS === "full" || top.inlineContext(dom) || /[^ \t\r\n\u000c]/.test(value)) {
        if (!preserveWS) {
          ...
          for (let i2 = 0; i2 < lines.length; i2++) {
            ...
            if (lines[i2])
              this.insertNode(schema2.text(lines[i2]), marks, !/\S/.test(lines[i2]));
          }
          value = "";
        } else {
  ...
  function chainCommands(...commands2) {
    return function(state, dispatch, view2) {
      for (let i2 = 0; i2 < commands2.length; i2++)
        if (commands2[i2](state, dispatch, view2))
          return true;
      return false;
    };
  }
    ...
    updateChildren(view2, pos) {
      ...
      iterDeco(this.node, this.innerDeco, (widget, i2, insideNode) => {
        if (widget.spec.marks)
          updater.syncToMarks(widget.spec.marks, inline, view2, i2);
        else if (widget.type.side >= 0 && !insideNode)
          updater.syncToMarks(i2 == this.node.childCount ? Mark.none : this.node.child(i2).marks, inline, view2, i2);
        updater.placeWidget(widget, view2, off);
      }, (child, outerDeco, innerDeco, i2) => {
        updater.syncToMarks(child.marks, inline, view2, i2);
        let compIndex;
        ...
        else if (updater.updateNextNode(child, outerDeco, innerDeco, view2, i2, off)) ;
        else {
  ...
  var ViewTreeUpdater = class {
    ...
    syncToMarks(marks, inline, view2, parentIndex) {
  ...
  editHandlers.keypress = (view2, _event) => {
    let event = _event;
    if (inOrNearComposition(view2) || !event.charCode || event.ctrlKey && !event.altKey || mac4 && event.metaKey)
      return;
  ...
  function runHandlerOnContext(view2, propName, pos, inside, event) {
    ...
    for (let i2 = $pos.depth + 1; i2 > 0; i2--) {
      if (view2.someProp(propName, (f) => i2 > $pos.depth ? f(view2, pos, $pos.nodeAfter, $pos.before(i2), event, true) : f(view2, pos, $pos.node(i2), $pos.before(i2), event, false)))
        return true;
    }
    return false;
  }
  ...
  function parseBetween(view2, from_, to_, addedNodes) {
  ...
  function readDOMChange(view2, from2, to, typeOver, addedNodes) {
    ...
    let sel = view2.state.selection;
    let parse = parseBetween(view2, from2, to, addedNodes);
  ...
  var paragraph = (text) => schema.nodes.paragraph.create(null, schema.text(text));
  var initialDocument = schema.nodes.doc.create(null, [
