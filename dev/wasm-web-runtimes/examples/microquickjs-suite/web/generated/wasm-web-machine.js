...
const ELEMENTS = new Set([
    "a", "br", "button", "div", "footer", "h1", "header", "img", "input", "label", "main",
    "li", "meta", "p", "section", "span", "textarea", "title", "ul"
]);
const SVG_ELEMENTS = new Set(["circle", "ellipse", "line", "path", "svg"]);
const SVG_IMAGE_ATTRIBUTES = new Set([
    "aria-hidden", "class", "cx", "cy", "d", "fill", "opacity", "r", "rx", "ry", "stroke",
    "stroke-linecap", "stroke-width", "viewBox", "x1", "x2", "y1", "y2"
]);
const ATTRIBUTES = new Set([
    "aria-autocomplete", "aria-expanded", "aria-hidden", "aria-label", "aria-live", "aria-multiline",
    "aria-pressed", "aria-selected", "autocapitalize",
    "autocomplete", "autocorrect", "class", "content", "contenteditable", "data-language",
    "cx", "cy", "d", "data-href", "fill", "form", "href", "id", "main-field", "name", "opacity", "r",
    "placeholder", "readonly", "role", "rx", "ry", "spellcheck", "stroke", "stroke-linecap", "stroke-width",
    "tabindex", "title", "translate", "type", "value", "viewBox", "writingsuggestions", "x1", "x2",
    "y1", "y2"
]);
const CSS_PROPERTIES = new Set([
    "-webkit-user-modify", "-webkit-user-select", "align-items", "animation", "animation-duration",
    ...
    "border-top-width", "box-shadow", "box-sizing", "caret-color", "color", "column-gap",
    "contain", "container-type", "content", "cursor", "direction", "display", "filter", "flex-wrap", "font",
    ...
    "font-weight", "grid-template-columns", "height", "inset-inline-end", "inset-inline-start",
    ...
    "min-height", "min-width", "object-fit", "opacity", "outline", "outline-color", "outline-offset", "outline-style",
    "outline-width", "overflow-anchor", "overflow-x", "overflow-y", "padding-bottom",
    "padding-left", "padding-right", "padding-top", "pointer-events", "position",
    "resize", "right", "row-gap", "tab-size", "text-align", "text-decoration-color", "text-decoration-line",
    "text-decoration", "text-decoration-style", "text-decoration-thickness", "top",
    "text-overflow", "text-transform", "transform", "transform-style", "perspective",
    "transition",
...
const CSS_VALUE_FUNCTIONS = new Set([
    "blur", "brightness", "clamp", "cubic-bezier", "drop-shadow", "linear-gradient",
    "min", "radial-gradient", "repeat", "rgba", "rotateY", "saturate", "steps", "var"
]);
const SAFE_SELECTOR = /^[#.a-z0-9_[\]="' >-]+$/i;
...
function svgImageAttributeAllowed(name, value) {
    if (typeof value !== "string")
        return false;
    if (["cx", "cy", "r", "rx", "ry", "stroke-width", "x1", "x2", "y1", "y2"]
        .includes(name))
    ...
    if (name === "class")
        return value === "" ||
            /^[a-z_][a-z0-9_-]{0,127}(?:\s+[a-z_][a-z0-9_-]{0,127})*$/i.test(value);
    if (name === "d")
...
async function installFont(font, bytes, allowFont) {
    if (!font || !(bytes instanceof Uint8Array) ||
        !/^[a-z][a-z0-9 ]{0,63}$/i.test(font.family) ||
        font.style !== "normal" ||
        !/^\d{1,4}(?: \d{1,4})?$/.test(font.weight) ||
        !["auto", "block", "fallback", "optional", "swap"].includes(font.display)) {
        fail("font record is not allowed");
    }
    const weights = font.weight.split(" ").map(Number);
    if (weights.some((weight) => weight < 1 || weight > 1000) ||
...
function isVirtualHref(value) {
    ...
    if (value === "#/")
        return true;
    if (!/^#\/[a-z0-9][a-z0-9._/-]{0,254}(?:#[a-z0-9][a-z0-9_.:-]{0,127})?$/i.test(value) ||
        value.includes("//")) {
        return false;
    }
...
function createBridge(target, options) {
    if (target !== document)
        fail("0.1 supports document mode only");
    const chunks = [];
    const leases = [];
    ...
    const navigationAllowed = (value) => {
        if (typeof options.allowNavigate === "function") {
            return options.allowNavigate(value) === true;
        }
        if (options.allowNavigate === "fragment")
            return isVirtualHref(value);
    ...
    function trace(type, detail) {
        ...
        try {
            ...
            // Diagnostics must never alter the sandbox they observe.
        }
    }
    ...
    function reference(value) {
        ...
        const known = reverse.get(value);
        if (known !== undefined) {
            leases[known]++;
            options.onReferenceLease?.(known, leases[known], value);
            return ["r", known];
        }
        ...
        (chunks[chunk] ||= [])[id % CHUNK_SIZE] = value;
        leases[id] = 1;
        options.onReferenceLease?.(id, leases[id], value);
        reverse.set(value, id);
        options.onReferenceCreate?.(id, value);
        if (instrument)
            trace("reference-create", {
                id,
                kind: value?.constructor?.name || typeof value,
                node: value instanceof Node ? value.nodeName : undefined,
            });
        return ["r", id];
    }
    function dereference(id) {
        ...
        const value = chunks[Math.floor(id / CHUNK_SIZE)]?.[id % CHUNK_SIZE];
        if (value === undefined)
    ...
    function insideDocument(node) {
        return node === document || node === document.head || node === document.body ||
            (node instanceof Node && document.documentElement.contains(node));
    }
    function get(object, name) {
        ...
        if (object === document && name === "activeElement") {
            return object.activeElement && insideDocument(object.activeElement) ?
                reference(object.activeElement) : null;
        }
        ...
        if (object === document &&
            ["devicePixelRatio", "innerHeight", "innerWidth", "pageXOffset", "pageYOffset"]
                .includes(name)) {
        ...
        if (object instanceof Selection &&
            ["anchorOffset", "focusOffset", "isCollapsed", "rangeCount"].includes(name))
            return object[name];
        ...
        if (object instanceof MutationRecord &&
            ["attributeName", "oldValue", "type"].includes(name))
            return object[name];
        if (object instanceof MutationRecord &&
            ["nextSibling", "previousSibling", "target"].includes(name)) {
            const node = object[name];
            return node && insideDocument(node) ? reference(node) : null;
        }
        if (object instanceof MutationRecord &&
            ["addedNodeCount", "removedNodeCount"].includes(name)) {
        ...
        if (object instanceof DOMRectReadOnly &&
            ["bottom", "height", "left", "right", "top", "width", "x", "y"].includes(name)) {
            // Preserve subpixel layout while keeping the compact wire integer-only.
        ...
        if (object instanceof CSSStyleDeclaration &&
            ["direction", "height", "overflow", "paddingBottom", "paddingTop", "position",
                "whiteSpace", "width"].includes(name))
            return object[name];
        ...
        if (object instanceof Element &&
            ["clientHeight", "clientWidth", "scrollHeight", "scrollWidth"].includes(name)) {
            return object[name];
        }
        if (object instanceof HTMLElement &&
            ["offsetHeight", "offsetWidth", "scrollLeft", "scrollTop"].includes(name)) {
            return Math.round(object[name]);
        }
        if (object instanceof HTMLTextAreaElement &&
            (name === "selectionStart" || name === "selectionEnd"))
            return object[name];
        ...
        if (object instanceof Event && ["altKey", "ctrlKey", "defaultPrevented", "metaKey",
            "shiftKey", "type"].includes(name))
            return object[name];
        if (object instanceof FocusEvent && name === "relatedTarget") {
            return object.relatedTarget instanceof Node && insideDocument(object.relatedTarget) ?
                reference(object.relatedTarget) : null;
        }
        ...
        if (object instanceof MouseEvent &&
            ["button", "buttons", "clientX", "clientY", "detail"].includes(name)) {
            return object[name];
        }
        if (object instanceof Event && name === "target") {
            return object.target instanceof Node && insideDocument(object.target) ?
                reference(object.target) : null;
        }
    ...
    function set(object, name, next) {
        if (object instanceof DataTransfer && ["dropEffect", "effectAllowed"].includes(name) &&
            typeof next === "string" && next.length <= 16) {
            object[name] = next;
            return null;
        }
        if (object instanceof HTMLElement && ["scrollLeft", "scrollTop"].includes(name) &&
        ...
        if (object instanceof HTMLElement && name === "tabIndex" &&
            Number.isInteger(next) && next >= -1 && next <= 0) {
            object.tabIndex = next;
            return null;
        }
        if (object instanceof HTMLInputElement && name === "value" && typeof next === "string" &&
            next.length <= 2048) {
            object.value = next;
            return null;
        }
        if (object instanceof HTMLTextAreaElement && name === "value" && typeof next === "string" &&
            next.length <= 2048) {
            object.value = next;
            return null;
        }
        ...
        if (object instanceof HTMLElement && name === "className" && typeof next === "string" &&
            /^(?:(?:[a-z_][a-z0-9_-]*)(?:\s+[a-z_][a-z0-9_-]*)*)?$/i.test(next)) {
            object.className = next;
            return null;
        }
        if (object instanceof HTMLElement && ["textContent", "title"].includes(name) &&
            typeof next === "string") {
            object[name] = next;
            return null;
        }
        if (object instanceof Comment && name === "textContent" && typeof next === "string" &&
            next.length <= 4096) {
            object.textContent = next;
            return null;
        }
        if (object instanceof Text && (name === "nodeValue" || name === "textContent") &&
            typeof next === "string" && next.length <= 1024 * 1024) {
            object[name] = next;
            return null;
        }
    ...
    function call(object, name, args) {
        ...
        if (object instanceof CSSStyleDeclaration && name === "removeProperty" &&
            args.length === 1 && typeof args[0] === "string" && isAllowedProperty(args[0])) {
        ...
        if (object === document && name === "getComputedStyle" && args.length === 1 &&
            args[0] instanceof Element && insideDocument(args[0])) {
            return reference(getComputedStyle(args[0]));
        }
        if (object === document && ["measureRect", "measureClientRects"].includes(name) &&
            args.length === 1 && (args[0] instanceof Range ||
            args[0] instanceof Element && (insideDocument(args[0]) || ownedNodes.has(args[0])))) {
            const rects = name === "measureRect" ? [args[0].getBoundingClientRect()] :
                Array.from(args[0].getClientRects());
        ...
        if (object === document && name === "scrollBy" && args.length === 2 &&
        ...
        if (object === document && name === "mutationObserve" && args.length === 3 &&
            args[0] instanceof Node && Number.isInteger(args[1]) && Number.isInteger(args[2])) {
            const flags = args[1];
            if (flags < 1 || flags > 31)
        ...
        if (object === document && name === "timer") {
            if (!Number.isInteger(args[0]) || args[0] < 0 || args[0] > 2147483647 ||
                !Number.isInteger(args[1]) || intervals.size >= 1024)
                fail("timer is not allowed");
            const id = nextInterval++;
            intervals.set(id, setInterval(() => deliver(args[1]), args[0]));
            return id;
        }
        if (object === document && name === "timerCancel" && args.length === 1 &&
        ...
        if (object === document && name === "cleanupOpportunity" && args.length === 2) {
            if (!Number.isInteger(args[0]) || args[0] < 0 || args[0] > 60000 ||
                !Number.isInteger(args[1]) || cleanupCallbacks.size >= 1024) {
            ...
            const handle = typeof requestIdleCallback === "function"
                ? requestIdleCallback(run, { timeout: args[0] }) : setTimeout(run, Math.min(args[0], 50));
            cleanupCallbacks.set(id, handle);
            return id;
        }
        if (object === document && name === "animationFrame" && args.length === 1 &&
            Number.isInteger(args[0])) {
            return requestAnimationFrame(() => deliver(args[0]));
        }
        if (object === document && name === "cancelAnimationFrame" && args.length === 1 &&
            Number.isInteger(args[0])) {
            cancelAnimationFrame(args[0]);
            return null;
        }
        if (object === document && name === "task" && args.length === 1 &&
            Number.isInteger(args[0])) {
            setTimeout(() => deliver(args[0]), 0);
            return null;
        }
        if (object === document && name === "windowListen") {
            if (!["beforeprint", "blur", "focus", "resize", "scroll"].includes(args[0]) ||
                !Number.isInteger(args[1])) {
                fail("window event listener is not allowed");
            }
            target.defaultView.addEventListener(args[0], (event) => deliver(args[1], event));
            return null;
        }
        if (object instanceof Event &&
            ["preventDefault", "stopImmediatePropagation", "stopPropagation"].includes(name) &&
            args.length === 0) {
            object[name]();
            return null;
        }
        if (object instanceof DataTransfer && ["getData", "setData"].includes(name)) {
            ...
            if (name === "setData" && args.length === 2 && typeof args[1] === "string" &&
                args[1].length <= 2 * 1024 * 1024) {
        ...
        if (object instanceof Node && name === "contains" && args.length === 1 &&
            (args[0] === null || args[0] instanceof Node)) {
            return object.contains(args[0]);
        }
        if (object instanceof Node && name === "childNodes" && args.length === 0 &&
            insideDocument(object)) {
            return reference(Array.from(object.childNodes));
        }
        if (object instanceof Node && name === "childNodeReferences" && args.length === 0 &&
            insideDocument(object)) {
        ...
        if (Array.isArray(object) && name === "item" && args.length === 1 &&
            Number.isInteger(args[0]) && args[0] >= 0 && args[0] < object.length) {
            return reference(object[args[0]]);
        }
        if (object instanceof MutationRecord &&
            ["addedNodeAt", "removedNodeAt"].includes(name) && args.length === 1 &&
            Number.isInteger(args[0])) {
        ...
        if (object instanceof Element && name === "getAttribute" && args.length === 1 &&
            typeof args[0] === "string" && (isAllowedAttribute(args[0]) || args[0] === "style")) {
        ...
        if (object instanceof Selection && ["collapse", "extend"].includes(name) &&
            args.length === 2 && args[0] instanceof Node &&
            (insideDocument(args[0]) || ownedNodes.has(args[0])) &&
        ...
        if (object instanceof Selection && name === "addRange" && args.length === 1 &&
            args[0] instanceof Range) {
            object.addRange(args[0]);
            return null;
        }
        if (object instanceof Selection && name === "getRangeAt" && args.length === 1 &&
            args[0] === 0 && object.rangeCount) {
            return reference(object.getRangeAt(0));
        }
        if (object instanceof Range && ["setStart", "setEnd"].includes(name) &&
            args.length === 2 && args[0] instanceof Node &&
            (insideDocument(args[0]) || ownedNodes.has(args[0])) &&
            Number.isInteger(args[1]) && args[1] >= 0) {
            object[name](args[0], args[1]);
        ...
        if (object instanceof Range && name === "collapse" && args.length === 1 &&
        ...
        if (object === document && name === "getElementById") {
            ...
            const found = document.getElementById(args[0]);
        ...
        if (object instanceof Element && name === "setAttribute") {
            ...
            if (object instanceof HTMLMetaElement &&
                !((args[0] === "name" && args[1] === "viewport") ||
                    (args[0] === "content" && args[1] === "width=device-width, initial-scale=1"))) {
            ...
            if (args[0] === "name" && !(object instanceof HTMLMetaElement) &&
                (!(object instanceof HTMLInputElement || object instanceof HTMLButtonElement ||
                    object instanceof HTMLTextAreaElement) ||
                    !/^[a-z_][a-z0-9_.:-]{0,127}$/i.test(args[1]))) {
            ...
            if (args[0] === "main-field" &&
                (!(object instanceof HTMLInputElement) || args[1] !== "true")) {
                fail("main-field is only allowed on a search input");
            }
            if (object instanceof SVGElement &&
                (!SVG_IMAGE_ATTRIBUTES.has(args[0]) || !svgImageAttributeAllowed(args[0], args[1]))) {
            ...
            if (args[0] === "href" &&
                (!(object instanceof HTMLAnchorElement) || !navigationAllowed(args[1]))) {
            ...
            if (["contenteditable", "spellcheck", "writingsuggestions"].includes(args[0]) &&
                !["true", "false"].includes(args[1])) {
                fail(`${args[0]} value is not allowed`);
            }
            if (args[0] === "aria-autocomplete" &&
                !["both", "inline", "list", "none"].includes(args[1])) {
                fail("aria-autocomplete value is not allowed");
            }
            if (["autocapitalize", "autocomplete", "autocorrect"].includes(args[0]) &&
                !["off", "on"].includes(args[1])) {
                fail(`${args[0]} value is not allowed`);
            }
        ...
        if (object instanceof Node && name === "closest") {
            if (args.length !== 1 || typeof args[0] !== "string" ||
                !/^\.[a-z_][a-z0-9_-]{0,63}$/i.test(args[0])) {
        ...
        if (object instanceof Element &&
            ["querySelector", "querySelectorAll", "querySelectorAllReferences"].includes(name) &&
            args.length === 1 && typeof args[0] === "string" && args[0].length <= 128 &&
            SAFE_SELECTOR.test(args[0])) {
            if (name === "querySelector") {
                const found = object.querySelector(args[0]);
                return found ? reference(found) : null;
            }
        ...
        if (object instanceof Element && name === "hasAttribute" && args.length === 1 &&
            typeof args[0] === "string" && isAllowedAttribute(args[0])) {
            return object.hasAttribute(args[0]);
        }
        if ((object instanceof Element || object instanceof Range) &&
            name === "getClientRects" && args.length === 0) {
        ...
        if (object instanceof Node && ["append", "appendChild", "replaceChildren"].includes(name)) {
            if (object === document.head && args.some((node) => !(node instanceof HTMLMetaElement || node instanceof HTMLTitleElement ||
        ...
        if (object instanceof Node && name === "insertBefore" && args.length === 2 &&
            args[0] instanceof Node && (args[1] === null || args[1] instanceof Node)) {
            // A projected editor can retire an ephemeral browser node before the
            // guest replays a batch that used it as the tail anchor. The guest still
            // owns ordering; a retired tail anchor therefore means append, not a
            // sandbox failure.
            const next = args[1] !== null && args[1].parentNode === object ? args[1] : null;
            object.insertBefore(args[0], next);
            return reference(args[0]);
        }
        if (object instanceof Node && name === "removeChild" && args.length === 1 &&
            args[0] instanceof Node && args[0].parentNode === object) {
            object.removeChild(args[0]);
            return reference(args[0]);
        }
        if (object instanceof Node && name === "removeChild" && args.length === 1 &&
            args[0] instanceof Node && args[0].parentNode === null) {
            // Mutation delivery may have already retired an ephemeral projection.
            return reference(args[0]);
        }
        if (object instanceof Element && name === "remove") {
            object.remove();
            return null;
        }
    ...
    function listen(object, type, callback, capture = false) {
        const elementEvent = object instanceof Element &&
            ["beforeinput", "blur", "change", "click", "compositionend", "compositionstart",
                "compositionupdate",
                "contextmenu", "copy", "cut", "dragend", "dragenter", "dragleave", "dragover",
                "dragstart", "drop", "focus", "focusout", "input",
                "keydown", "keyup", "mousedown", "mouseleave", "mousemove", "mouseup", "mousewheel",
                "paste",
                "pointerdown", "pointerenter", "pointerleave", "pointermove", "pointerup", "scroll",
                "submit", "touchend", "touchmove", "touchstart", "wheel"].includes(type);
        const documentEvent = object === document &&
            ["mousemove", "mouseup", "selectionchange", "visibilitychange"].includes(type);
        if ((!elementEvent && !documentEvent) ||
            !Number.isInteger(callback))
    ...
    function operation(item) {
        ...
        if (instrument)
            trace("operation", {
                ...
                object: item[1],
                name,
                argumentCount: item[0] === 3 ? (item[3] || []).length : undefined,
                value: item[0] === 2 ? traceValue(item[3]) : undefined,
                arguments: item[0] === 3 ? (item[3] || []).map(traceValue) : undefined,
            });
        try {
            if (item[0] === 0)
            ...
            if (instrument)
                trace("operation-error", {
                    code: item[0], object: item[1], name, message: error?.message || String(error),
                });
            throw error;
        }
    }
    ...
    function installStylesheet(bytes, targetStyle = null, replace = false) {
        ...
        for (let ruleIndex = 0; ruleIndex < itemCount; ruleIndex++) {
            ...
            if (itemKind === 2) {
                ...
                const font = {
                    family: reader.text(), style: reader.text(), weight: reader.text(),
                    display: reader.text(),
                };
                ...
                reader.at += length;
                pendingChecks.push(installFont(font, bytes, options.allowFont));
                continue;
            }
            if (itemKind !== 1)
            ...
            for (let declarationIndex = 0; declarationIndex < declarationCount; declarationIndex++) {
                ...
                for (let tokenIndex = 0; tokenIndex < tokenCount; tokenIndex++) {
                    ...
                    if (kind === 0)
                        ...
                        value += JSON.stringify(reader.text());
                    else if (kind === 5)
                        value += ",";
                    else if (kind === 6)
                        value += "/";
                ...
                declaration.setProperty(property, value, important ? "important" : "");
                if (!declaration.getPropertyValue(property)) {
                    fail(`CSS value ${value} for ${property} in rule ${ruleIndex + 1} was not accepted`);
                }
                declarations.push(`  ${property}: ${value}${important ? " !important" : ""};`);
            }
    ...
    function renderSvg(bytes) {
        ...
        function node(depth) {
            ...
            for (let index = 0; index < attributeCount; index++) {
                ...
                if (!SVG_IMAGE_ATTRIBUTES.has(name) || value.length > 2048 ||
                    !svgImageAttributeAllowed(name, value)) {
            ...
            return `<${tag}${namespace}${attributes.length ? ` ${attributes.join(" ")}` : ""}>` +
                `${children.join("")}</${tag}>`;
        }
        const source = node(0);
    ...
    function msg(offset, capacity) {
        if (capacity === 0) {
            if (offset === 0)
                return 0; // The null message starts or wakes the host.
            ...
            if (remaining === 0) {
                chunks[Math.floor(id / CHUNK_SIZE)][id % CHUNK_SIZE] = undefined;
        ...
        if (capacity >= 4 && memory[offset] === 68 && memory[offset + 1] === 85 &&
            memory[offset + 2] === 76 && memory[offset + 3] === 83) {
            if (!stamp || stamp.length > capacity)
                fail("stamped guest data is unavailable or too large");
            memory.set(stamp, offset);
            return stamp.length;
        }
        if (capacity >= 6 && memory[offset] === 68 && memory[offset + 1] === 85 &&
            memory[offset + 2] === 76 && memory[offset + 3] === 69) {
        ...
        // Operations in this message originated in the guest. Do not echo their
        // projected DOM mutations back as external edits on the next microtask.
    ...
    function deliver(callback, event = null) {
        ...
        try {
            ...
            if (instrument)
                trace("event-error", {
                    callback, eventType: event?.type || null, message: error?.stack || String(error),
                });
            throw error;
        }
        if (instrument)
    ...
    return {
        imports: { host: { msg, now: () => performance.now() } },
        async connect(nextInstance) {
            if (instance)
                fail("host is already running a guest");
            instance = nextInstance;
            if (typeof instance?.exports?.onmsg !== "function" ||
...
export default class WasmWebMachine {
    #bridge;
    #connected = false;
    #instance;
    constructor(module, target = document, options = {}) {
        ...
        this.#bridge = createBridge(target, options);
    ...
    onmsg = async (length = 0) => {
        if (!this.#connected) {
            if (length !== 0) {
                throw new Error("wasm-web-machine: first message must be the null message");
            }
            this.#connected = true;
            await this.#bridge.connect(this.#instance);
            return;
        }
        return this.#instance.exports.onmsg(length);
    };
}
//# sourceMappingURL=wasm-web-machine.js.map