...
export function renderCommandPalette(model) {
  ...
  return `<button class="command-trigger" type="button" data-command-open aria-label="${escapeHtml(model.placeholder)}"><svg class="command-trigger__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4-4"></path></svg><span>${escapeHtml(model.placeholder)}</span><kbd data-command-shortcut>Ctrl K</kbd></button>
