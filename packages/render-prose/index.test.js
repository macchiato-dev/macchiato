...
test('renders italic with *asterisks*', () => {
  const document = build('Hello *world*.');
  assert.ok(document.toHTML().includes('<em>world</em>'));
});
...
test('renders bold with **asterisks**', () => {
  const document = build('Hello **world**.');
  assert.ok(document.toHTML().includes('<strong>world</strong>'));
});
...
test('inline markup in heading', () => {
  const document = build('# Hello **world**');
  assert.ok(document.toHTML().includes('<h1>Hello <strong>world</strong></h1>'));
});
