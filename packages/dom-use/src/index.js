...
export class DomUse {
    ...
    refillGas(ownerDocument, now = Date.now()) {
        ...
        if (!intervalMs || !amount)
            return;
        const elapsed = Math.max(0, now - ownerDocument.gas.lastRefill);
        const intervals = Math.floor(elapsed / intervalMs);
        if (intervals <= 0)
            return;
        ownerDocument.gas.available = Math.min(ownerDocument.gas.capacity, ownerDocument.gas.available + intervals * amount);
    ...
    urlRuleFor(tagNameOrNode, attr) {
        ...
        if (globalUrls === true || globalUrls === false)
            return globalUrls;
        if (globalUrls[`${tag}.${name}`] !== undefined)
            return globalUrls[`${tag}.${name}`];
        if (globalUrls[name] !== undefined)
            return globalUrls[name];
        if (globalUrls["*"] !== undefined)
            return globalUrls["*"];
        return undefined;
    }
    fragmentRuleFor(tagNameOrNode) {
        ...
        for (const nodeUrls of nodeUrlRules) {
            if (nodeUrls === false)
                return false;
            if (nodeUrls?.fragments !== undefined)
                return nodeUrls.fragments;
        }
        if (globalUrls === false)
            return false;
        if (globalUrls?.fragments !== undefined)
...
export function createDomDocument(domUse) {
    return new GuestDocument(domUse);
}
export function sanitizeDomHtml(domUse, html, options = {}) {
    return domUse.sanitizeHTML(html, options);
}
export function setDomInnerHtml(domUse, node, html) {
    domUse.setInnerHTML(node, html);
}
export function assertDomAttribute(domUse, node, name, value) {
