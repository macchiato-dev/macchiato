...
test("Resources.co edge account creates organizations and projects in a real browser", async (t) => {
  ...
  assert.match((await versionsButton.textContent()).replace(/\s+/g, " ").trim(), /^Just now1/);
...
test("Resources project save controls publish titled latest versions", async (t) => {
  ...
  await page.waitForFunction(() => document.querySelector("[data-current-version]")?.textContent === "Just now");
  assert.match(await page.locator("[data-current-version]").textContent(), /^Just now$/);
  await page.locator("[data-project-versions-proxy]").click();
