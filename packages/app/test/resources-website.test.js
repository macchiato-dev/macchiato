...
test("Resources.co edge account creates organizations and projects in a real browser", async (t) => {
  ...
  await assert.doesNotReject(page.locator(".project-view__identity").getByText("tiny-tools/", { exact: true }).waitFor());
  ...
  assert.match((await versionsButton.textContent()).replace(/\s+/g, " ").trim(), /^Current Version3/);
  ...
  await assert.doesNotReject(page.locator("[data-project-versions] .project-editor__version-count", { hasText: "4" }).waitFor());
