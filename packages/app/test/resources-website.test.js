...
test("Resources.co edge account creates organizations and projects in a real browser", async (t) => {
  ...
  await assert.doesNotReject(page.locator("[data-project-versions] .project-editor__version-count", { hasText: "8" }).waitFor());
  ...
  assert.equal(await page.locator("[data-project-versions] .project-editor__version-count").textContent(), "8");
  ...
  await page.waitForFunction(() => document.querySelector("[data-project-save]")?.textContent === "Unsaved changes");
