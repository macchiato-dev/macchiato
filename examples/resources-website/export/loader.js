const __bundlerScript = document.currentScript;
const __bundlerBase = new URL('.', __bundlerScript.src);

document.addEventListener('DOMContentLoaded', async function() {
  const loading = document.getElementById('__bundler_loading');
  function setStatus(msg) { if (loading) loading.textContent = msg; }
  async function readBundleText(path) {
    const response = await fetch(new URL(path, __bundlerBase));
  ...
  try {
    const [manifestText, templateText, extResourcesText] = await Promise.all([
      readBundleText('manifest.json'),
      readBundleText('template.json'),
      readBundleText('ext-resources.json'),
    ]);
    ...
    const extResources = JSON.parse(extResourcesText);
    const resourceMap = {};
